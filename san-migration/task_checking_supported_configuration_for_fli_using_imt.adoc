...
. Select the solution *Foreign LUN Import (FLI) backend Interoperability*.
. Select the *FAS model* and *ONTAP version* to determine the supported configurations.
. For frontend supported host configurations, click *build end to end view with ONTAP SAN host*.
