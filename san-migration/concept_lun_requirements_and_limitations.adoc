...
* The foreign LUN must be marked foreign on the destination array to prevent assignments from ONTAP.
* The foreign LUN must be in an import relationship before starting import.
* The LUN must be the same size as the foreign LUN .This requirement is taken care of during the LUN creation steps.
* The foreign LUN block size must be 512b. NetApp LUNs support only 512b block size.
* The LUN must not be expanding or contracting.
* The LUN must be mapped to at least one igroup.
* The NetApp LUN should be brought offline before creating a relationship. However, after the LUN relationship is created, it can be brought back online in case of online FLI.
