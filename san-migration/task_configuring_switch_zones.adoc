...
.Steps
. Back up the existing zone sets on each switch in the production and migration fabric.
...
. Create the zone and add it to the zone set in production fabric A.
+
...
. Create the zone and add it to the zone set in production fabric B.
+
...
. Activate the zone set in production fabric B.
